...
HERE = Path(__file__).resolve().parents[1] / "assurance_boundaries_v3"
...
def load_api(target):
    ...
    if api.DecisionTokenSigner.TOKEN_VERSION != "1.3.0":
        raise RuntimeError("unsupported target interface")
    return api


def assured(api):
    return api.DecisionTokenSigner.TOKEN_VERSION == "1.3.0"
...
def submit(api, instance, token, request, now):
    ...
    payload = api.DecisionTokenSigner(TEST_KEY).verify(token, now=now)
    if any(receipt.get(k) != payload[k] for k in (
        "token_id", "envelope_digest", "idempotency_key", "actor", "policy_version", "policy_digest"
    )):
        raise RuntimeError("simulated receipt changed a signed authority binding")
...
def run_request(api, policies, case, now, path):
    ...
    if decision.decision_token:
        ...
        policy_digest = digest(json.dumps(policies, sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False).encode())
        exact = exact and payload["policy_digest"] == policy_digest and payload["assurance_digest"] == decision.assurance_digest
...
def measure(target):
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="cerberus-rc-") as temporary:
    ...
    return {
        "benchmark": "release-candidate-v1", "now": corpus["now"],
        ...
        "limits": "Reuses frozen v3 request-bound attestations; strict PolicyBundle schema metadata added, token 1.3.0 binds policy and assurance. Simulated effects only; no independent, continuous or adaptive evaluation.",
        "summary": {
