...
def build():
    ...
    for record in source["cases"]:
        ...
        if case_id.startswith("H"):
            # Isolate approval failures: genuine evidence binds the submitted
            # request, while approval proofs retain the adversarial construction.
            _, exact_evidence = fixture_assurance(envelope)
            bundle = replace(bundle, evidence=exact_evidence.evidence)
        cases.append({
    ...
    return {k: v for k, v in source.items() if k != "cases"} | {
        ...
        "attestation_profile": "cerberus.evidence.v2: full request binding; cerberus.approval.v1",
