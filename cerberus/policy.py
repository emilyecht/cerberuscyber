...
class PolicyBundle:
    ...
    def __post_init__(self) -> None:
        # Direct construction and dataclasses.replace must not bypass validation
        # or let a supplied digest name different rules.
        try:
            data = json.loads(self._canonical, object_pairs_hook=_reject_duplicate_keys)
            validate_policy_bundle(data)
            canonical = _canonical_json(data)
        except (TypeError, json.JSONDecodeError) as exc:
            raise PolicyBundleError("invalid canonical policy bundle") from exc
        if canonical != self._canonical:
            raise PolicyBundleError("policy bundle is not canonical")
        if self.digest != hashlib.sha256(canonical.encode("utf-8")).hexdigest():
            raise PolicyBundleError("policy bundle digest does not match rules")
